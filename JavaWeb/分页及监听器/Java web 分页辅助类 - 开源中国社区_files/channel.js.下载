var osc = (function(window) {
	...
	var osc = {
		...
		show_tips : function(target, text, opt) {
			...
			return target.poshytip && target.poshytip(opt).poshytip('show');
		},
		...
		close_recommend:function(user_code){
			if(confirm("确认关闭城市圈推荐提醒？")){
				jQuery.post("/action/group/close_recommend",{"user_code": user_code},function(){
					location.reload();
				});
				/*
